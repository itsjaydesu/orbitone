...
function getDamperBuffer(midi: number): number {
  if (midi <= 48)
    return 0.15 // bass — heavy dampers
  if (midi <= 72)
    return 0.08 // mid
...
function normalizePreStartPedalEvents(pedalEvents: PedalEvent[]) {
  ...
  const latestPreStartEvent = pedalEvents.toReversed()
...
export function generateBeautifulPianoPiece(numMeasures: number = 32, bpm: number = 100): MusicData {
  const notes: NoteEvent[] = []
  let timeInSeconds = 0
